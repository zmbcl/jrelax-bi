...
$.fn.BiTable = function (options) {
    ...
    function initEvents() {
        ...
        //选中整行事件
        $table.find(".rownum").off("click");
        $table.find(".rownum").on("click", function () {
            obj.unSelectAll();
            var tr = $(this).parent("tr");
            obj.selectRow(tr.index());
        });
        //选中列事件
        $table.find(".colnum").off("click");
        $table.find(".colnum").on("click", function () {
            obj.unSelectAll();
            var tr = $(this).parent("tr");
            obj.selectCol($(this).index());
        });
    }
    ...
    this.selectRow = function (idx) {
        if (idx > 0) {
            select($table.find("tr").eq(idx).find("td:gt(0)"));
        }
    }

    //选择列
    this.selectCol = function (idx) {
        if (idx > 0) {
            $table.find("tr:gt(0)").each(function (i, n) {
                select($(n).find("td:eq(" + idx + ")"));
            });
        }
    }
