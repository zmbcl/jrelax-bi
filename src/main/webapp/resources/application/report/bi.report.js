...
function BiReport(opt) {
    ...
    this.renderCondition = function () {
        ...
        $.each(conditionEl, function (i, n) {
            ...
            var result = false;
            if (conditionSymbol == "==") {
                result = eval(ns.String.format("\"%s\" %s \"%s\"", [td.text(), conditionSymbol, conditionValue]));
            } else {
                if (isNumber(conditionValue) && isNumber(td.text())) {
                    result = eval(td.text() + conditionSymbol + conditionValue);
                }
            }
            if (result) {
