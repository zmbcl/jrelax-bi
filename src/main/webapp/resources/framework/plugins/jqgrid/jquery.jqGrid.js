	...
	styleUI : {
		...
		Bootstrap : {
			...
			base : {
				...
				icon_refresh: "glyphicon-refresh",
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp,"refresh"+tp], btc=getstyle(stylemodule, 'pgButtonBox', true, 'ui-pg-button'),
						pot = [($.jgrid.getRegional(ts,"defaults.pgfirst",ts.p.pgfirst) || ""),
								...
								($.jgrid.getRegional(ts,"defaults.pgrefresh",ts.p.pgrefresh) || "")];
				if(dir==="rtl") { 
					po.reverse();
					pot.reverse();
				}
				...
				pgl += sep;
				pgl += "<td id='"+po[4]+"' class='"+btc+"' title='"+ pot[4] +"'" +"><span " + getstyle(stylemodule, 'icon_refresh',false, iconbase)+"></span></td>";
				pgl += sep;
			} else if (pginp !== "") {
				pgl += pginp;
			}
			...
			if(ts.p.pgbuttons===true) {
				...
				$("#refresh"+$.jgrid.jqID(tp)).click(function () {
					$(ts).reload();
				});
			}
			if(ts.p.pginput===true) {
